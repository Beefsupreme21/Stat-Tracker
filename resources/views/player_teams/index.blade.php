<x-layout>
    <div class="mx-auto mb-4 md:flex md:items-center md:justify-between md:w-4/5">
        <div class="min-w-0 flex-1">
            <h1 class="text-2xl font-bold leading-7 text-gray-900 ml-3 sm:truncate sm:text-3xl sm:tracking-tight">{{ $player->name }}'s Teams</h1>
        </div>
        <div class="mt-4 flex md:mt-0 md:ml-4">
            <a href="/players/{{ $player->id }}" class="ml-3 inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Back to player page</a>
        </div>
    </div>
                            ...
                            @foreach ($player->teams as $team)
                                <li class="py-4">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <form method="POST" action="/players/{{ $player->id }}/teams/{{ $team->id }}">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <li class="py-4">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div class="inline-flex items-center rounded-full border border-gray-300 bg-white px-2.5 py-0.5 text-sm font-medium leading-5 text-gray-700 shadow-sm hover:bg-gray-50">
                                            <form action="/players/{{ $player->id }}/teams" method="POST">
                                                @csrf
